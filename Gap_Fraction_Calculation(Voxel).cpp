...
{
	//输入的原始点云数据（x,y,z） 
	ifstream input("H:\\ondalabLiDARdata\\Single Tree LAI\\whole_tree\\tree1.txt");

	//输出的栅格属性 
	ofstream output_data_attribute("H:\\ondalabLiDARdata\\Single Tree LAI\\whole_tree\\tree1_output_data_attribute.txt");
	int num = 0;
	while (input >> x >> y >> z >> temp >> temp >> temp)
	{
		pc[num][0] = x;
		pc[num][1] = y;
		num++;
	...
	output_data_attribute << "Input the point cloud file path as " << endl << "H:\\ondalabLiDARdata\\Single Tree LAI\\whole_tree\\tree1.txt" << endl << endl;
	...
	//输出的栅格 
	ofstream output_data_mapping("H:\\ondalabLiDARdata\\Single Tree LAI\\whole_tree\\tree1_output_data_mapping.txt");
